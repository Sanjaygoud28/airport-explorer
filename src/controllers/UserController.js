...
async function signupUser(req, res) {
  try {
    const { name, email, mobile, displayName, password } = req.body; //only required fields

    if (!name || !email || !mobile || !password) {
      throw new Error("Name ,email and password are all required");
    }

    const existingUserWithEmail = await User.exists({ email });
...
async function loginUser(req, res) {
  try {
    ...
    console.log("successfully login", existedUser);
